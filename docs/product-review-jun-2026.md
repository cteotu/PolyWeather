...
| # | 问题 | 原优先级 |
|---|------|------|
| 1 | Pro 加载阻塞看板 → 只显示精简顶栏+加载状态 | 🔴 P0 |
| 2 | 免费用户 Scan 失败无重试 → 添加重试按钮 | 🔴 P0 |
| 3 | Detail Panel 超时无反馈 → 显示超时警告 | 🔴 P0 |
| 4 | 无新用户引导 → 3 步 WelcomeOverlay | 🟡 P1 |
| 5 | 付费墙无预览 → 功能说明文案 | 🟡 P1 |
| 6 | 密码重置缺失 → 登录页忘记密码 + Supabase 重置 | 🟡 P1 |
| 7 | 邮箱验证提示不明 → 登录失败时提示检查验证邮件 | 🟡 P1 |
| 8 | 公告不可关闭 → ✕ 按钮永久隐藏 | 🟢 P2 |
| 9 | 无反馈入口 → 顶栏 Telegram 反馈按钮 | 🟢 P2 |
| 10 | entitlement-required 页面简陋 → 品牌化设计 + 导航 | 🟢 P2 |
| 11 | 订阅帮助仅中文 → 中英双语 | 🟢 P2 |
| 12 | 账户中心个人设置 → 决策不做，钱包+Telegram 已足够 | 🟢 P2 |
